<!-- resources/views/general_qualifiers/index.blade.php -->
                ...
                <tbody>
                    @foreach ($qualifiers as $qualifier)
                        <tr class="border-t hover:bg-gray-50 transition duration-200">
                            ...
                            <td class="px-6 py-4 text-sm text-gray-600">{{ $qualifier->note }}</td>
                            <td class="px-6 py-4 text-sm text-gray-600">{{ $qualifier->rating }}</td>
                            <td class="px-6 py-4 space-x-2">
                                ...
                                <form action="{{ route('general_qualifiers.destroy', $qualifier) }}" method="POST" style="display:inline;">
                                    @csrf
                                    @method('DELETE')
                                    <x-button-start type="submit" onclick="return confirm('Are you sure you want to delete this general qualifier?');">
                                        Delete
                                    </x-button-start>
                                </form>
